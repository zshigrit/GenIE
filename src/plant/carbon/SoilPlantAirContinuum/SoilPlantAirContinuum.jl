...
"""
note1: have to add "include"; change the ".." to "."; 
comment out "define the constants" in CanopyLayers.jl 
"""

include("../CanopyLayers/CanopyLayers.jl")  # added by zheng for .. does not work for unknown reason 
...
include("../Photosynthesis/Photosynthesis.jl") # add by zheng
...
include("../PlantHydraulics/PlantHydraulics.jl") # add by zheng
...
include("../StomataModels/StomataModels.jl") # add by zheng
...
weather = DataFrame(CSV.File("./weather/gs_sample.csv"))
output = create_dataframe(FT, weather::DataFrame)

annual_simulation!(node, photo_set, weather, output)
CSV.write("output_fluxes.csv", output)
end 
