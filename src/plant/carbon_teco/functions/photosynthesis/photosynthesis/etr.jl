...
# note 1: to calculate J, here use min(IpsII, Jmax) instead of solving the 
# colimitation equations
# ! done reviewing !
###############################################################################
"""
...
function leaf_ETR!(
            photo_set::C3Cytochrome{FT},  # involved in electron transport chain
            leaf::Leaf{FT}
) where {FT<:AbstractFloat}
...
function leaf_ETR!(
    ...
    _Jp = PSII_frac * maxPSII * APAR; # this is IpsII in Bonan 
    _J  = min(_Jp, Jmax);
