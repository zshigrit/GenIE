...
#    const rad2photon = FT(4.6);
    ...
    # for phosphorus
    include("soil/types/IPPar.jl")
    include("soil/types/OPPar.jl")
    include("soil/types/MPPools.jl")
...
#    include("ecosystem/readinweather.jl")
    ...
    include("ecosystem/plantsoilcnp.jl")
    include("ecosystem/annualsimulationCNP.jl") # directly include a function
    include("ecosystem/initparameters.jl")

# for phosphorus

    include("soil/phosphorus/InitIP.jl")
    include("soil/phosphorus/InitOP.jl")
    include("soil/phosphorus/IPpoolUpdate.jl")
    include("soil/phosphorus/OPPoolUpdate.jl")
    include("soil/phosphorus/NPPool.jl")
# some constants
...
# instantialize and intialize parameters/ conditions 
    ...
    ppools = InitPPools(soil.OC, soil.par_add)
    rCP = InitCP(soil.par_add)
    ip = InitIP(ip,ippar)
...
# simulation
    annual_simulationCNP!(soil,plant,weather,_weather,swc,ts,lai,pH,
                       output_plant,output_pools,output_fluxes,
                       ippar,oppar,ip,ppools,rCP)
